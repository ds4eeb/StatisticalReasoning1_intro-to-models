...
Welcome! This is the first statistical reasoning activity. We will learn how to implement basic bayesian models using the `brms` package.
...
To start, let's plot our response variable carapace width (`size`) on the y axis with our predictor variable `latitude` on the x axis.
...
2.    Is the effect reasonably different from zero? In other words, does the confidence interval intersect with zero?
